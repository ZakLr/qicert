...
**Last verified:** 2026-09-13 ~19:30 UTC, after the N9-GO doc update.
**HEADLINE — the capability gate is PASSED:** N9 repair training (LoRA r=8,
1200 steps, train-split only) on the compressed seed-0 model, then the
full-protocol confirm (`results/N9C/5085c6d99434_repair-confirm-seed0/`):
**0.4535 accuracy at 2.459× honest ratio — GO** (bar was ≥0.3968 at ≥2×).
Wilson CI [0.4506, 0.4564] non-overlapping above the FT baseline's
[0.4439, 0.4497]; certificates re-derived from the merged weights (168/168
exact). The training-free route's NO-GO (0.164 @2.536×, mode collapse) is
kept as the honest middle of the arc. Both docs now tell this story:
`docs/technical-report-v2.pdf` (8 pp, 0 errors/overfull — abstract, Table 1
N9 row, Results, Limitations, Expected Impact, Conclusion, Appendix A all
updated; compression-plane figure includes the N9 GO point, regenerated by
`scripts/make_report_figures.py`) and `docs/concept-proposal.pdf` (3 pp —
prototype, impact, and validation-plan sections updated to the GO verdict).
**Novelty sweep (2026-09-13, web):** no prior work found that combines
TT compression + certified runtime guard on VLA/robot policies. Nearest
neighbors and our deltas: SafeVLA (safety alignment via constrained RL —
training-time, no certificate); AEGIS/VLSA (plug-in safety layer — no
compression, no bound); certified-training lines (COLBERT/Randomized-Smoothing
works — probabilistic, not exact per-layer bounds); TT-compression surveys
2025/2026 (no certificates); BitVLA/QuantVLA-style quantization (no bounds);
CompactifAI (TT for LLM compression, no runtime enforcement). Our specific
combination — exact per-layer bounds from compressed cores + refusing guard
+ repair-then-recertify loop + honest negative-result characterization on a
...
acc ≥ 0.3968 (= FT − 0.05). **N9-CONFIRM 2026-09-13 ~18:06 UTC: GO (seed 0,
full-split)** — repaired model 0.4535 @2.459× honest ratio (Δ +0.0068 vs FT,
CI [0.4506, 0.4564] non-overlapping above FT's [0.4439, 0.4497]).
Artifact `results/N9C/5085c6d99434_repair-confirm-seed0/`. Caveats: single
seed (need 1–2 for mean±std); re-derived certs are exact per-layer dense
norms (168/168) but the naively chained ball is log10L=71.1 (mean 2.65/layer
over 168 layers) — vacuous end-to-end, as it would be for ANY 168-deep model
incl. the dense baseline. Ship per-layer exactness + guard/estimator stack,
never the chained ball as an operational number.
...
# N9 repair training (RAN 2026-09-13: prefix 0.1586 -> 0.4519, full confirm
# pending - see below):
