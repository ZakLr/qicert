...
legend, workspace refs removed, 0 errors/overfull). Compression-plane figure
includes the N9 GO point (`scripts/make_report_figures.py`).
**Novelty sweep (2026-09-13, web):** no prior work found that combines
TT compression + certified runtime guard on VLA/robot policies — "novel
combination", not "novel field" (`archive/dev-notes/PHASE2-CASE.md` on clean).
**Website plan (2026-09-13 ~21:45 UTC, user-locked):** stunning competition site,
not slop — lab-instrument palette (Ink/Paper/Ruler/Signal/Cert/Warn),
Instrument Serif + Suisse Int + JetBrains Mono, proof-sheet layout, signature
**manifest ruler** hash tape. 7 plates: Hero, Honest wall, Repair→Confirm,
Certificates, Guard (interactive 5-case + entropy window), Safety+Monitor,
Reproduce. Two variants to deliver (single-page proof-sheet vs 7 routed
plates, same content). Full `results/` publishable with privacy scrub; JS
guard demo allowed. Plan saved at `docs/website-plan.md` (clean branch).
...
- **Lean 4 (do it — high value, low cost):** machine-checked proof of the
  ...
  by `lake build`; artifact link in the report. Weakest-link algebra is
  simple enough for a one-file proof (~150–300 lines). NOT a whole-model
  proof (that would be overclaiming).
- **C++ (defer to Phase 2, say so):** justified only by latency numbers we
  do not have; Python passes the 100 ms benchmark on measured paths.
- **CUDA-Q (already in, do not expand):** the IQAE amplification simulation
  already uses it; reusing it just to claim the logo invites judge questions.
- **Cheap add-ons with real value:** output-diversity check in the guard
  (directly motivated by our own mode-collapse finding — collapse makes a
  certificate vacuous, so the guard should refuse degenerate outputs too);
  CI-gated reproducibility badge (fresh-venv smoke on push).
**Final remaining GPU experiment:** baseline seeds 1–2 (`--only n1v2`, ~1–2 h);
then optionally N9 for seeds 1–2 (compress+repair+confirm per seed) to make
the GO row multi-seed — see §9 for the exact plan and decision rule.
...
acc ≥ 0.3968 (= FT − 0.05). **N9-CONFIRM: GO, now replicated.** Seed 0 —
0.4535 @2.459× honest ratio (Δ +0.0068 vs its FT 0.4468 [0.4439, 0.4497]);
seed 1 — 0.4609 @2.459× (Δ +0.0639 vs its FT 0.3970, hardest lift); mean
0.4572, both GO, 168/168 re-derived certs each, CIs non-overlapping above
their FTs. Caveat: end-to-end chained ball log10L ~71 vacuous for any
168-deep model — ship per-layer exactness + guard/estimator stack, never the
chained ball.
...
3. **N1v2 baseline seeds 1 & 2 — done, 3-seed mean±std:** seeds 0–2
...
# confirm-s1 GO; repair-s2 done, confirm-s2 running):
...
8. **N9 design (2026-09-13):** compresses the seed-0 FT weights (NOT the base
    ...
    **N9 RESULT 2026-09-13 ~16:27 UTC (seed 0, prefix 600 batches/10603 tokens):
    acc_pre 0.1586 → acc_post 0.4519 (Δ +0.2932), vs FT 0.4468 (Δ +0.0051,
    inside 1 SE — parity, not superiority; CIs overlap).**
    Objective caveats: (1) prefix-only — full 6496-batch confirm on
    `results/N9-repaired/repaired_seed0.pt` still pending; (2) `cert_sound
    168/168` covers the pre-repair compressed cores, NOT the LoRA-merged
    weights (`lora_delta_spectral_max` 4.86) — the deployed ball must be
    re-derived from the merged weights before any certificate claim on the
    repaired model; (3) the saved handoff is a 1.26 GB dense merge, so the
    2.536× ratio only holds if deployed as cores+residual+adapters, not as
    the saved dense file; (4) repair cost (1200 LoRA steps) must be charged
    to our arm under the matched-budget rule.
...
## 9. Multi-seed plan and package-improvement plan (2026-09-13, post-N9-GO)

### 9a. The per-seed pipeline (what to run after `n1v2` finishes)

The GO row currently rests on seed 0 only. The challenge asks for mean ± std
over ≥3 independent runs. Full pipeline per seed s ∈ {1, 2}, in this order:

1. **Baseline FT** — stage `n1v2` (trains seeds 1 and 2 together, ~1–2 h).
   Produces `results/N1v2-ckpt/seed{1,2}.pt` + full-split baselines.
   *Decision gate:* if either seed's FT accuracy lands far from 0.4468
   (say <0.40), STOP and report the FT seed variance honestly — do not
   silently proceed to compress a bad baseline.
2. **Calibration for the seed** — the N2R calib file is seed-0-specific
   (`results/N2R/calib_seed0.npz`). Per-seed compression needs
   `calib_seed{s}.npz`. Check whether the N9 module parameterizes the calib
   path (`QICERT_N2R_CALIB` env or similar) before running; if it does not,
   add the env var (small change, one file) rather than reusing seed-0 stats.
3. **N9 per seed** — compress seed s at the SAME confirmed recipe
   (frac 0.50, rr64, absmax, no-mixed), prefix-eval the collapsed model,
   train the r=8 adapter 1200 steps (same lr, same seed protocol), merge,
   save `results/N9-repaired/repaired_seed{s}.pt`, then run `n9-confirm`
   for seed s (full protocol + re-derived certs + honest ratio). ~1.5–2 h
   per seed.
4. **Report row:** mean ± std of {0.4535, acc_seed1, acc_seed2} at the
   matched ratio; likewise the FT row. If a seed fails the gate (one bad
   seed out of three), report all three numbers and say the verdict is
   2-of-3 — never drop a seed silently.

Total added GPU: ~3–4 h for both seeds (compress+train+confirm ≈ 1.5–2 h
each, sequential, one job at a time). If time runs out before the deadline:
ship the seed-0 GO row + baseline seeds 1–2 (mean ± std on the REFERENCE
row) and state that N9 replication is queued — the challenge rewards the
honest partial over the padded whole.

### 9b. Package improvements (decided, with rationale)

| Item | Verdict | Why / cost |
|---|---|---|
| **Lean 4 proof artifact** | DONE 2026-09-13 (`lean/`, commit `d29c157`) | `lake build` clean, zero `sorry`: `prod_le_prod_of_pointwise` (bound composition, ℕ-formalized with the ℝ scope boundary stated in-file), `guard_sound`/`guard_complete`/`guard_negative_margin_rejects_all` (mirror the Z3 checks), `Audit.lean` witnesses foundation-only axioms. Toolchain + Batteries pinned; own CI workflow scaffolded by `lake new`. ℝ field lemmas would need Mathlib (deferred: heavy build, correctly skipped mid-training). |
...
| **Conformal risk control upgrade** | DO if time (cheap, natural) | We already measure split-conformal coverage at 3 levels; upgrading to bounded cumulative risk (Angelopoulos-style) is the deployment-grade version judges associate with real assurance. Reuses the existing calibration fixture. |
| **Hardware-pathway estimate (1 page)** | DO if time | Qubit count / depth / shot budget for the IQAE readout as an appendix — the proposal already promises this for Phase 2; a static estimate is cheap credibility. |
| **CI-gated reproducibility badge** | DO if time | GitHub Action: fresh venv, run smoke suite, badge in README. Judges can see it's green at submission time. |
| **Attention-replacement (linear/SSM) arm** | DEFER | Interesting Phase-2 lever; touching the backbone now risks invalidating the frozen-protocol comparability of every measured row. |

Priority order for remaining sessions: seeds pipeline (9a) → Lean artifact →
guard diversity check → CI badge. Items marked DEFER are Phase-2 material and
belong in the proposal's Phase-2 section only (already there).
