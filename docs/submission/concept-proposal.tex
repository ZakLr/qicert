...
%  report (docs/submission/technical-report-v2.pdf) and the public repo. Every number is
...
Technical report: \texttt{docs/submission/technical-report-v2.pdf} (7~pages, in repo)
\end{center}
...
hash-frozen split): reference accuracy $0.4319 \pm 0.0303$ (seeds 0.4468,
0.3970, 0.4520); calibrated INT8 at
...
and repair training on the compressed model (arm N9, below) clears it
($0.4496 \pm 0.0137$ at $2.46\times$, replicated across 3 seeds,
certificates re-derived from the final deployed weights). Both
...
consumed 10.19~GPU-hours of scored wall-clock on a single laptop-class GPU
(NVIDIA RTX~5060, 8~GB), including fine-tuning (1{,}200 steps), all
compression arms, full-split evaluations, the estimator fixtures, and latency
profiling (full accounting in the repository's resource declaration). Peak training
...
$0.4319 \pm 0.0303$ and the INT8 comparator at $1.997\times$ define the bar
the tensor route must clear; certificates already hold on every compressed
layer at every tested ratio; the estimator layer already delivers the
...
at mean $\pm$ std over 3 seeds (N9: $2.46\times$, $0.4496 \pm 0.0137$, GO).
Phase 2 hardens it into an
engineering result: larger backbones, closed-loop rollouts (mean $\pm$ standard deviation
...
$0.4496 \pm 0.0137$ at $2.46\times$ honest ratio over 3 seeds (0.4535,
0.4609, 0.4343; seed 0 alone: 95\% Wilson interval $[0.4506, 0.4564]$,
non-overlapping above its reference's $[0.4439, 0.4497]$), with
certificates re-derived from the final deployed weights (168/168 exact) at
a cost of one SVD per layer. Phase 2 escalates the remaining levers in cost
...
the present model has in total). Metrics: pre-registered gate, already met
at mean $\pm$ std over 3 seeds on both rows (baseline $0.4319 \pm 0.0303$,
repair $0.4496 \pm 0.0137$).
...
\item \textbf{Technical report (8 pages):} \texttt{docs/submission/technical-report-v2.pdf}
