...
\textbf{Capability claim (Phase-1 experiment completed; Phase-2 escalation).}
The Phase-1 configuration search over the repaired tensor route finished
under the pre-registered gate (ratio $\ge 2\times$ AND accuracy $\ge 0.3968$):
best certified point $0.1640$ at $2.536\times$ --- NO-GO, with the failure
mechanism identified (mode collapse to the modal action token; the
agreement-vs-compression curve is bimodal, not gradual). Phase 2 pulls the
three levers this diagnosis isolated, in cost order: trained repair of the
compressed weights (a low-rank adapter on the compressed model --- expected
to recover the largest fraction, since the same $4.4$M-parameter budget
lifted the base model from $0.11$ to $0.4468$; certificates re-derived from
the final weights at a cost of one SVD per layer), activation-aware rank
allocation, and backbone scale (a $1.5$--$7$B backbone, whose weights carry
the redundancy a $0.5$B LoRA-tuned model lacks --- at $2.5\times$ a $3$B
model retains more absolute capacity than the present model has in total).
Metrics unchanged: pre-registered gate, mean $\pm$ standard deviation over
3 independent seeds (seeds 1--2 queued at submission time; seed 0 measured).
