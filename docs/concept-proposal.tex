...
Technical report: \texttt{docs/technical-report-v2.pdf} (8~pages, in repo)
\end{center}
...
action; key properties also proved in Lean~4). A guard demo transcript shows: clean load SERVE,
out-of-ball action REFUSE, tampered-certificate REFUSE, NaN-bound REFUSE,
missing-manifest REFUSE, plus an entropy gate flagging collapsed streams.
...
pre-registered capability gate (ratio $\ge 2\times$ AND accuracy within
$0.05$ of reference) is answered: the training-free route misses it
($0.1640$ at $2.536\times$, failure mechanism identified as mode collapse),
and repair training on the compressed model clears it ($0.4535$ at
...
(NVIDIA RTX~5060, 8~GB), including fine-tuning (1{,}200 steps), all compression
arms, full-split evaluations, the estimator fixtures, and latency profiling
(full accounting in the repository's resource declaration). Peak training
...
per model, a capability the quantized baseline cannot produce at any
ratio.
...
train-split episodes only (contamination-checked against the frozen split),
merged into the compressed model, re-evaluated on the full protocol:
$0.4535$ at $2.46\times$ honest ratio, Wilson 95\% CI $[0.4506, 0.4564]$
non-overlapping above the uncompressed reference's $[0.4439, 0.4497]$, with
...
rather than framed). AI research agents were used for engineering work
...
claim-to-artifact provenance map (Appendix~A) with artifact-generated
figures.
