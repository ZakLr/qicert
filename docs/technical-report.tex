...
$O(\sum_i r_i^2 n_i)$ per step without ever forming $W$. Chaining through the
...
ratio $\kappa = L(\F)\,/\,\sup_{x\in B}\norm{\nabla \F(x)}_2$ (denominator by
power iteration on the full TT network) and prints it at every Pareto point.
...
$O(k_{\mathrm{it}}\sum_i r_i^2 n_i)$ with $k_{\mathrm{it}}$ iterations, the
dense matrix never formed. Both properties are preserved for QTT (log-mode)
...
\end{equation}
The oracle is a \emph{rule-based} STL evaluator over the spec library
(Q21) --- a cheap polynomial program, not a neural rollout --- and the
amplitude register holds only the verdict bit $[\rho<0]$ (or $\log_2 b$ bits
for $b$-bin robustness histograms). The neural network is never placed inside
the simulated quantum circuit; oracle queries count STL evaluations, and the
full query budget is printed per benchmark. Layer 3 is \emph{offline
evaluation-time}: the $\le 100$\,ms real-time budget governs the runtime path
of Sec.~\ref{sec:arch} (compiled diagonal kernels + monitor), which never
executes an amplitude-estimation loop.
...
per benchmark. \emph{Simulation honesty:} the Grover depth $1/\sqrt{p}$ is
real simulator cost at our classical budgets --- that is precisely why the race
against the strong classical arm at matched query budget is pre-registered
(R4), and the claim never assumes quantum hardware. If Q loses to C-strong at
our budgets, we print that; Layers 1--2 stand independently. Adjacent QAE
applications include structural reliability \cite{tabarraei}, flow-based
failure sampling \cite{quantfp}, and MPPI control \cite{mppi}; our in-domain
delta is VLAM/AD safety certification fused with exact structural
certificates.
...
in-body so no stronger claim can be imputed. Runtime parity with classical
baselines is an acceptance target, not an assumption: E-comp-3 ($\le 100$\,ms
at the full compiled table) and the N14 clean-env bench are the measured
evidence.
