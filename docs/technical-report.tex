...
The full AE circuit uses $\log_2|S| + O(1)$ qubits, keeping simulated
state-vector cost at $O(|S|)$ scale, tractable at our scenario counts under
...
violation probability on the cheap oracle above; C-strong estimates the
empirical failure probability from true rollouts. They are compared at matched
\emph{total} budget, including the printed oracle compilation cost, and the
crossover point (oracle complexity at which the classical arm wins) is a
reported output of N6. If Q loses, we print that (pre-registered R4); Layers
1--2 stand independently. Adjacent QAE applications include structural
