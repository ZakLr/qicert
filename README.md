...
- **`prod_le_prod_of_pointwise`**: the composition theorem, stating that the certified bound of a chain of layers is the product of the per-layer bounds. This is the mathematical spine of every certificate we ship.
...
- **Per-layer exact ≠ end-to-end certified.** Chaining 168 exact bounds multiplies into a vacuous number; that's true of *any* 168-layer model, including the uncompressed one. We ship per-layer exactness + the runtime guard, and we say so.
